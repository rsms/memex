...
func init() {
	...
	go func() {
		...
		defer log.RootLogger.Sync() // sync logger after returning
		...
		for _, fn := range handlers {
			go func(fn ExitHandler) {
				defer func() {
					if r := recover(); r != nil {
						log.Error("panic in RegisterExitHandler function: %v\n", r)
				...
				if err := fn(ctx); err != nil {
					if err != context.DeadlineExceeded && err != context.Canceled {
						log.Error("RegisterExitHandler function: %v", err)
					}
					// cancel the shutdown context
					cancel()
				} else {
		...
		for range handlers {
			...
			case <-ctx.Done():
				...
				if ctx.Err() == context.DeadlineExceeded {
					log.Warn("shutdown timeout (%s)", timeout)
				}
				if exitCode == 0 {
					exitCode = 1
				}
